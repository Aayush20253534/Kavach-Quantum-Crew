## Current KAVACH integration snapshot

`blockchain/` is a separate signing/proof service. The main Express backend talks to its HTTP gateway using `BLOCKCHAIN_GATEWAY_URL` + `BLOCKCHAIN_GATEWAY_KEY`; the gateway alone receives `CHAIN_RPC_URL`, `CONTRACT_ADDRESS`, and `ISSUER_PRIVATE_KEY`. `TrustAnchor.sol` supports proof-oriented issue/revoke/anchor/verify operations. Application payloads are canonicalized/hashed and privacy-scanned before chain submission; the public chain is not the primary KAVACH database.

Background anchor jobs in the main backend allow domain work to persist even when the chain/gateway is temporarily unavailable. Retry/attempt limits are configuration-driven.
...
## Current snapshot and realtime integrity workflow
