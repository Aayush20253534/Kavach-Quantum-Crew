...
export function NotificationsDropdown() {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <div className={`absolute top-[calc(100%+12px)] right-0 w-80 sm:w-96 bg-white border border-slate-200 rounded-xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.15)] z-50 overflow-hidden transition-all duration-300 origin-top-right ${
          ...
          {notifications.map((notification) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-5 flex gap-4 border-b border-slate-50 hover:bg-slate-50/80 transition-colors ${unread ? 'bg-white' : 'bg-slate-50/40'}`}
              >
                <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm border bg-slate-50 text-slate-600 border-slate-200">
                  <Icon className="w-5 h-5" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    <p className={`text-[13px] font-bold truncate ${unread ? 'text-slate-900' : 'text-slate-600'}`}>{notification.title}</p>
                    <span className="text-[10px] font-bold text-slate-400 whitespace-nowrap">{relativeTime(notification.createdAt)}</span>
                  </div>
                  <p className={`text-[12px] leading-relaxed ${unread ? 'text-slate-600 font-medium' : 'text-slate-500'}`}>{notification.message}</p>
                </div>
