...
The Join Trip page uses the device camera (or an uploaded image) to scan the group invitation QR generated by the trip leader. The QR contains an opaque invitation token, never tourist personal data. The client first calls the preview endpoint and only joins after the user confirms the validated trip details.

Install frontend dependencies after applying this patch:
