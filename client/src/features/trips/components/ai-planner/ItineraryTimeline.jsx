      ...
      {days.map((dayPlan) => (
        <div key={dayPlan.day} className="relative">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="bg-slate-900 text-white px-4 py-1.5 rounded-lg text-sm font-black uppercase tracking-widest">
              Day {dayPlan.day}
            </div>
          ...
          <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-200">
            {(!dayPlan.places || dayPlan.places.length === 0) ? (
              <div className="relative flex items-center gap-6 group pl-2">
                ...
                <div className="flex-1 p-4 rounded-2xl border border-slate-100 bg-slate-50 text-slate-500 italic text-sm font-medium">
                  Free Day — No specific activities planned.
                </div>
              </div>
            ) : (
              dayPlan.places.map((place, idx) => (
                <div key={idx} className="relative flex items-start gap-6 group pl-2">
                  {/* Timeline Node */}
                  ...
                  {/* Activity Card */}
                  <div className="flex-1 p-5 rounded-2xl bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow group/card relative">
                    
                    {/* Action buttons (visible on hover) */}
                    <div className="absolute top-4 right-4 opacity-0 group-hover/card:opacity-100 transition-opacity flex items-center gap-1">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start gap-5">
                      {place.thumbnail && (
                        <div className="w-full sm:w-28 h-40 sm:h-28 shrink-0 rounded-xl overflow-hidden bg-slate-100 border border-slate-200">
                          <img 
                            src={place.thumbnail} 
                            alt={place.name}
                      ...
                      <div className="flex-1 pr-12 sm:pr-16">
                        ...
                        <h4 className="text-lg font-black text-slate-900 mb-2 leading-snug">{place.name}</h4>
