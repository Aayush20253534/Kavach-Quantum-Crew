...
export const createIncidentService = ({ repository = incidentRepository, notifier = notificationService, clock = () => new Date() } = {}) => {
  const find = async (id) => {
  ...
  return Object.freeze({
    async ingestSafetyAlert(alert) {
      ...
      const incident = await repository.createFromSafetyAlert(alert, location); await notifier.incidentCreated(incident); return incident;
    },
    async triggerSos(userId, input) {
      ...
      await notifier.incidentCreated(result.incident);
      return result;
    },
    ...
    async acknowledge(actor, id, note) {
      ...
      await repository.createAudit({ actorId: actor.id, actorRole: actor.role, action: "INCIDENT_ACKNOWLEDGED", entityId: id }); await notifier.statusChanged(updated,"INCIDENT_ACKNOWLEDGED","Your incident has been acknowledged by emergency staff."); return updated;
    },
    async startResponse(actor, id, note) {
      ...
      const now = clock();
      const updated=await repository.transition(id,{status:"IN_PROGRESS",startedAt:now,...(incident.acknowledgedAt?{}:{acknowledgedAt:now,acknowledgedById:actor.id,acknowledgedByRole:actor.role})},{type:"RESPONSE_STARTED",actorId:actor.id,actorRole:actor.role,note}); await notifier.statusChanged(updated,"RESPONSE_STARTED","Emergency response has started for your incident."); return updated;
    },
    async resolve(actor, id, note) {
      ...
      await repository.createAudit({ actorId: actor.id, actorRole: actor.role, action: "INCIDENT_RESOLVED", entityId: id }); await notifier.statusChanged(updated,"INCIDENT_RESOLVED","Your incident has been resolved."); return updated;
    },
    async dismiss(actor, id, note) {
      ...
      const updated=await repository.transition(id,{status:"DISMISSED",dismissedAt:clock(),resolutionNote:note},{type:"DISMISSED",actorId:actor.id,actorRole:actor.role,note}); await notifier.statusChanged(updated,"INCIDENT_DISMISSED","The incident was dismissed by emergency staff."); return updated;
    },
    async assign(actor,id,responderId){staffOnly(actor,"INCIDENT_ASSIGN_FORBIDDEN");const incident=await find(id);if(["RESOLVED","DISMISSED"].includes(incident.status))throw ApiError.conflict("Closed incident cannot be assigned",{code:"INCIDENT_CLOSED"});const targetId=responderId??actor.id;if(actor.role===ROLES.DISASTER_MANAGER&&targetId!==actor.id)throw ApiError.forbidden("Disaster managers can only self-assign",{code:"INCIDENT_ASSIGN_OTHER_FORBIDDEN"});if(!await repository.findResponder(targetId))throw ApiError.notFound("Responder not found",{code:"RESPONDER_NOT_FOUND"});const updated=await repository.assign(id,targetId,ROLES.DISASTER_MANAGER,actor,clock());await repository.createAudit({actorId:actor.id,actorRole:actor.role,action:"INCIDENT_ASSIGNED",entityId:id,metadata:{responderId:targetId}});await notifier.assigned(updated);return updated;},
    async unassign(actor,id){staffOnly(actor,"INCIDENT_UNASSIGN_FORBIDDEN");const incident=await find(id);if(!incident.assignedToId)return incident;if(actor.role===ROLES.DISASTER_MANAGER&&incident.assignedToId!==actor.id)throw ApiError.forbidden("Only the assigned responder or an admin can unassign",{code:"INCIDENT_UNASSIGN_OTHER_FORBIDDEN"});return repository.unassign(id,actor,clock());},
    async addNote(actor,id,note){staffOnly(actor,"INCIDENT_NOTE_FORBIDDEN");await find(id);return repository.addNote(id,actor,note);},
  });
};
