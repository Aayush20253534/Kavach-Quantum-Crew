...
export const createDispatchService = ({ repository = dispatchRepository, publisher = realtimePublisher, emailer = emergencyEmailService, clock = () => new Date() } = {}) => {
  ...
  const ensureIncidentDispatchable = (incident) => {
    if (["RESOLVED", "DISMISSED"].includes(incident.status)) {
      throw ApiError.conflict("Closed incident cannot be dispatched", { code: "INCIDENT_CLOSED" });
    }
    if (incident.trip?.status !== "ACTIVE") {
      throw ApiError.conflict("Incident trip is no longer active", {
        code: "INCIDENT_TRIP_EXPIRED",
        details: { tripStatus: incident.trip?.status ?? "UNKNOWN" },
      });
    }
  };
  ...
  return Object.freeze({
    ...
    async create(actor, incidentId, input) { staffOnly(actor); const incident = await repository.findIncident(incidentId); if (!incident) throw ApiError.notFound("Incident not found", { code: "INCIDENT_NOT_FOUND" }); ensureIncidentDispatchable(incident); let unit = null; if (input.unitId) unit = await ensureAssignableUnit(input.unitId, input.unitType); const now = clock(); const dispatch = await repository.createDispatch({ incidentId, requestedUnitType: input.unitType, unitId: unit?.id ?? null, status: unit ? "ASSIGNED" : "REQUESTED", requestedById: actor.id, requestedByRole: actor.role, requestedAt: now, assignedAt: unit ? now : null }); if (unit) await repository.updateUnit(unit.id, { status: "DISPATCHED" }); return record(dispatch, actor, unit ? "ASSIGNED" : "REQUESTED", input.note, { unitId: unit?.id ?? null }); },
    async autoAssign(actor, incidentId, serviceType, input = {}) {
      ...
      ensureIncidentDispatchable(incident);
    ...
    async assign(actor, id, input) { staffOnly(actor); const current = await getDispatch(id); if (TERMINAL.has(current.status)) throw ApiError.conflict("Closed dispatch cannot be assigned", { code: "DISPATCH_CLOSED" }); ensureIncidentDispatchable(current.incident); if (current.unitId) throw ApiError.conflict("Dispatch already has an assigned unit", { code: "DISPATCH_ALREADY_ASSIGNED" }); const unit = await ensureAssignableUnit(input.unitId, current.requestedUnitType); const now = clock(); const updated = await repository.updateDispatch(id, { unitId: unit.id, status: "ASSIGNED", assignedAt: now }); await repository.updateUnit(unit.id, { status: "DISPATCHED" }); return record(updated, actor, "ASSIGNED", input.note, { unitId: unit.id }); },
    async transition(actor, id, status, note) { staffOnly(actor); const current = await getDispatch(id); if (TERMINAL.has(current.status)) return current; if (status === "CANCELLED") { const updated = await repository.updateDispatch(id, { status, cancelledAt: clock() }); if (current.unitId) await repository.updateUnit(current.unitId, { status: "AVAILABLE" }); return record(updated, actor, "CANCELLED", note); } if (!current.unitId) throw ApiError.conflict("Dispatch requires an assigned unit", { code: "DISPATCH_UNIT_REQUIRED" }); if (NEXT[current.status] !== status) throw ApiError.conflict("Invalid dispatch transition", { code: "DISPATCH_INVALID_TRANSITION", details: { current: current.status, requested: status } }); const now = clock(); const data = { status, ...(status === "DISPATCHED" ? { dispatchedAt: now } : {}), ...(status === "EN_ROUTE" ? { enRouteAt: now } : {}), ...(status === "ON_SCENE" ? { onSceneAt: now } : {}), ...(status === "COMPLETED" ? { completedAt: now } : {}) }; const updated = await repository.updateDispatch(id, data); let resolvedIncident = null; if (status === "COMPLETED") { await repository.updateUnit(current.unitId, { status: "AVAILABLE" }); resolvedIncident = await repository.resolveIncidentWhenResponsesComplete(current.incidentId, actor, now); } const recorded = await record(updated, actor, status, note); if (resolvedIncident?.status === "RESOLVED") publisher.publishIncidentUpdated?.(resolvedIncident, { type: "RESOLVED", actorId: actor.id, source: "FLEET_COMPLETION" }); return recorded; },
